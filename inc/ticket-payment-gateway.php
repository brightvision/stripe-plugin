...
class EM_Gateway_Stripe extends EM_Gateway { 
		...
		function booking_form(){
			/*if(!in_array(rt_group_name(), array("Starter","Explorer","Premium","VIP"))){*/
			
			echo get_option('em_'.$this->gateway.'_form');
			
			?>
			<div class="stripe-tkt-payment" style="display:none">
			...
			<div class="memberful-card-number">
			  <input autocomplete="cc-number" data-stripe="number" id="cc-number"  placeholder="Card number" type="text" value=""  name="stripe_card_num" >
			</div>
			<div class="memberful-card-expires">
			  <input autocomplete="cc-exp" data-stripe="exp" id="cc-expires" maxlength="9" name="ccard_exp" placeholder="MM / YY" type="text" value="">

			</div>
			<div class="memberful-billing-postal"> 
			<input autocomplete="postal-code" autocompletetype="postal-code" data-stripe="address_zip" id="cc-postal-code" placeholder="Zip code" type="text"></div>
			<div class="memberful-csc">
			  <input name="stripe_card_code" autocomplete="cc-csc" data-stripe="cvc" id="cc-cvc" placeholder="CVC" type="text" value="">
			</div>
			</div>
            <?php 
		}
		...
		function processStripe($EM_Booking){
			...
			/*$EM_Booking->add_error($amount."--".$amount_with_rt_fee."==".round($amout_with_stripe_fee,2));
				return false;*/
			...
			if(empty($_POST['ccard_exp']))
			{
				$EM_Booking->add_error(__('Please enter expiry month and year', 'em-pro'). '"');
				return false;
			}
			...
			if(!class_exists("\Stripe\Stripe")) require( 'stripe/init.php' );//including own library
			if($this->debug=='yes')
			...
			//$EM_Booking->add_error('Payment 3');return;
			if($this->debug=='yes'){
				EM_Pro::log(sprintf( __( 'Set Secret Key', 'emp_stripe' )));
			}
			try {
				$amount = $EM_Booking->get_price(false, false, true);
				$spacess=$EM_Booking->booking_spaces;
				$stp=$amount/$spacess;
				$rt_fee=6*$spacess;
				$amount_with_rt_fee=$amount+$rt_fee;
				$amout_with_stripe_fee=$amount_with_rt_fee+$amount_with_rt_fee*2.9/100+0.3;

				$amount=(int)$amout_with_stripe_fee;


				if($this->debug=='yes'){
				EM_Pro::log(sprintf( __( 'Credit Card token create', 'emp_stripe' )));
				}
				$ccard_exp=$_POST['ccard_exp'];
				$card_my=explode("/", $ccard_exp);

				$token_id = \Stripe\Token::create(array(
				 "card" => array( 
				 			"name" => $EM_Booking->get_person()->get_name(),
							"number" => $_POST['stripe_card_num'], 
							"exp_month" => (int)$card_my[0], 
							"exp_year" => (int)$card_my[1], 
